...
void main() {
  ...
  group('WeatherCubit', () {
    ...
    group('toggleFavoriteCity', () {
      blocTest<WeatherCubit, WeatherState>(
        'adds city to favorites if not present',
        build: () => weatherCubit,
        act: (cubit) => cubit.toggleFavoriteCity('Jakarta'),
        expect: () => <WeatherState>[
          WeatherState(favoriteCities: const ['Jakarta']),
        ],
      );

      blocTest<WeatherCubit, WeatherState>(
        'removes city from favorites if already present',
        build: () => weatherCubit,
        seed: () => WeatherState(favoriteCities: const ['Jakarta']),
        act: (cubit) => cubit.toggleFavoriteCity('Jakarta'),
        expect: () => <WeatherState>[
          WeatherState(favoriteCities: const []),
        ],
      );
    });
  });
